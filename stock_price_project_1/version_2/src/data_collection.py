...
y_train_no_fs, y_test = y_data[:train_size], y_data[train_size:]
...
print("Check for zero std in y_train_no_fs:")
std_y = np.std(y_train_no_fs)
print("std_y:", std_y)
print("Is zero or NaN in std_y?", std_y == 0 or np.isnan(std_y))
...
mu_2 = np.mean(y_train_no_fs,axis=0)
sigma_2 = np.std(y_train_no_fs, axis=0)
y_train = (y_train_no_fs - mu_2)/sigma_2
y_test = (y_test - mu_2)/sigma_2
